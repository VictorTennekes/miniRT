...
void	init_cy_children(t_object *cylinder, t_data *data)
{
	...
	*disk[1] = *cylinder;
	disk[0]->type = DS;
	disk[0]->pos[0] = vec_add(cylinder->pos[0],
		vec_multi(cylinder->vector, cylinder->height / 2));
	disk[1]->type = DS;
	disk[1]->pos[0] = vec_sub(cylinder->pos[0],
		vec_multi(cylinder->vector, cylinder->height / 2));
	if (!lst_new_back(&(data->objects), disk[0]) ||
		!lst_new_back(&(data->objects), disk[1]))
