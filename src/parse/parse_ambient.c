...
void	parse_ambient(char **info, t_data *data)
{
	if (data->mapinfo.amb_set == true)
		print_error("Multiple ambient ligh specifications", data);
	if (char_arrlen(info) != 3)
		print_error("Wrong amount of values given for ambient lighting", data);
	...
	if (data->mapinfo.amb_ratio > 1 || data->mapinfo.amb_ratio < 0)
		print_error("Invalid value given for ambient lighting", data);
